...
def standaardprijs(afstandKM):
    """
    Bepaalt de prijs van een rit doormiddel van de gegeven afstandKM.
    Telt getallen <0 niet mee en verrekent prijs vanaf 50 kilometer correct.
    """
    if afstandKM < 0:
        afstandKM = 0
...
def ritprijs(leeftijd, weekendrit, afstandKM):
    """
    Berekent de juiste kortingen als deze van toepassing zijn.
    """
    if leeftijd < 12 or leeftijd >= 65:
        if weekendrit:
            korting_ritprijs = standaardprijs(afstandKM) * 0.65  # 35% korting
        else:
            korting_ritprijs = standaardprijs(afstandKM) * 0.70  # 30% korting
            #feedback: maak een nested if statement ipv teveel in 1 if statement proppen
    else:
        if weekendrit:
            korting_ritprijs = standaardprijs(afstandKM) * 0.60  # 40% korting
        else:
            korting_ritprijs = standaardprijs(afstandKM)  # geen korting

    eindPrijs = round(korting_ritprijs, 2) #round() rondt het getal af naar 2 decimalen
    return eindPrijs


def development_code():
    # Plaats hieronder code om je functies tussentijds te testen. Bijv:
    print('dev test:', ritprijs(20, False, 51))
