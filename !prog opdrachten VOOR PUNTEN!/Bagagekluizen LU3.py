...
def aantal_kluizen_vrij():
    ...
    with open("kluizen.txt", "r") as kluizen: #zorgt ervoor dat het bestand vanzelf sluit
        kluisregels = kluizen.readlines() #berekent hoeveel lines de file bevat en dus hoeveel kluisjes er beschikbaar zijn
        kluizenVrij = 12 - len(kluisregels)
...
def nieuwe_kluis():
    """
    checkt of kluizen vrij zijn en zorgt dat er voor de juiste invoer gezorgd wordt.
    wijst de eerste vrije kluis toe aan de gebruiker en zet deze in de file.
    geeft de juiste foutmeldingen weer.
    """
    aantalKluizen = list(range(1, 13)) #maakt een list van 1-12


    with open("kluizen.txt", "r+") as kluizen:
        for kluis in kluizen.readlines():
            kluisnummer = int(kluis.split(";")[0]) #haalt de ";" weg en geeft kluisnummer de variabele van het kluisnummer in de list
            if kluisnummer in aantalKluizen:
                aantalKluizen.remove(kluisnummer) #haalt de kluis uit aantalKluizen

        if not aantalKluizen:  #als er geen vrije kluizen zijn
            print("Foutmelding -2: Alle kluizen zijn bezet.")
            return -2

        vrijKluisnummer = aantalKluizen[0]
        kluiscode = input("Geef een kluiscode van minimaal 4 tekens. Je code mag geen ';' bevatten!\nKluiscode: ")

        if ";" in kluiscode or len(kluiscode) < 4:
            print("Foutmelding -1. Geef een geldige invoer.")
            return -1
        else:
            kluizen.write(f"{vrijKluisnummer};{kluiscode}\n") #voegt het kluisnummer en de kluiscode toe aan het bestand op een nieuwe regel
            print(f"Gelukt! Je kluis met kluisnummer {vrijKluisnummer} is klaar voor gebruik.")
            return vrijKluisnummer
...
def kluis_openen():
    """
    opent de kluis als de juiste informatie ingevoerd is.
    """
    ...
    with open("kluizen.txt", "r") as kluizen:
        for kluis in kluizen.readlines():
            ...
            if nummer == kluisnummer and code == kluiscode:
                print("Kluis geopend!")
                return True

    print("Foutmelding: Onjuist kluisnummer of kluiscode.")
    return False
...
def kluis_teruggeven():
    """
    gebruiker kan de kluis teruggeven als deze de juist informatie invoert.
    de kluis wordt dan verwijdert uit de file.
    """
    kluisnummer = input("Voer uw kluisnummer in: ")
    kluiscode = input("Voer uw kluiscode in: ")
    ...
    with open("kluizen.txt", "w") as kluizen:
        for kluis in regels:
            nummer, code = kluis.strip().split(";")
            if nummer == kluisnummer and code == kluiscode:
                print("Kluis succesvol teruggegeven!")
                return True
            kluizen.write(kluis)  # schrijf alle andere kluizen terug naar de file
...
def development_code():
    ...
    while True: #zorgt ervoor dat je steeds terugkomt in het menu zonder de code opnieuw te hoeven starten
        print("-----------------------------------------------")
        ...
        menuKeuze = int(input("Wat wilt u doen?: "))

        if menuKeuze == 1:
            aantalVrij = aantal_kluizen_vrij()
            print("Er zijn", aantalVrij, "kluizen vrij.")
        elif menuKeuze == 2:
            nieuwe_kluis()
        elif menuKeuze == 3:
            kluis_openen()
            ...
            break  # Stop de loop en sluit het programma
        else:
            print("Fout. Geef een geldige invoer op.")

#bronnen die ik heb gebruikt: discuss.python.org / stackoverflow.com / geeksforgeeks.org / het boek: Introduction to Computing Using Python: An Application Development Focus
